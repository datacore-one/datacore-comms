...
1. **Preferred**: Call `plur_inject_hybrid` MCP tool with `prompt` = your task description and `scope` = `agent:voice-enforcer`
2. **Fallback**: If MCP is unavailable, read `.datacore/state/agent-engrams/voice-enforcer.md` for compiled engrams
